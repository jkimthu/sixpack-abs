...
%       AND using the same ranges of birth size as they do.
%       overall, quite like figure 28 save for this feature. uses data
%       structure of collected cell cycle stats build in first part of
%       figure 28.m script.
...
% last update: jen, 2018 May 31
% commit: plot added mass over birth size, using x axis scales shown in
%         Taheri et al 2015
...
for eCol = 1:length(environment_order)
    ...
    axis([lowerRange_raw_vol upperRange_raw_vol -1 6])
    title(condition)
    if eCol == 1
    ...
    axis([lowerRange_norm upperRange_norm -.1 3])
    title(condition)
    if eCol == 1
    ...
    axis([lowerRange_raw_l upperRange_raw_l -1 6])
    title(condition)
    if eCol == 1
