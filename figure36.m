...
%  Commit: monod and jensens expectations using growth rate in width
...
load('dWdtData_fullOnly.mat') % this data uses instantaneous points present after 3hrs, from full curves only
...
for e = 1:experimentCount
    ...
    G_low(counter) = dWdtData_fullOnly{index}{1,low}.mean;
    G_high(counter) = dWdtData_fullOnly{index}{1,high}.mean;
...
%axis([0 5 0 12])
title('test for strong day-to-day correlations in dW/dt')
xlabel('mean dW/dt in low')
ylabel('mean dW/dt in high')
...
%       will be normalized to average dW/dt from entire stable data set
...
load('dWdtData_fullOnly_normalized.mat') % both datasets are generated and saved in figure34.m script
...
%     end
    ...
    % raw data
    flucRates_raw(counter,fluc) = dWdtData_fullOnly{index}{1,fluc}.mean*3600;
    stableRates_raw(counter,low) = dWdtData_fullOnly{index}{1,low}.mean*3600;
    stableRates_raw(counter,ave) = dWdtData_fullOnly{index}{1,ave}.mean*3600;
    stableRates_raw(counter,high) = dWdtData_fullOnly{index}{1,high}.mean*3600;
    
    % normalized data
    flucRates_norm(counter,fluc) = dWdtData_fullOnly_normalized{index}{1,fluc}.mean*3600;
    stableRates_norm(counter,low) = dWdtData_fullOnly_normalized{index}{1,low}.mean*3600;
    stableRates_norm(counter,ave) = dWdtData_fullOnly_normalized{index}{1,ave}.mean*3600;
    stableRates_norm(counter,high) = dWdtData_fullOnly_normalized{index}{1,high}.mean*3600;
...
xlabel('fluctuating timescale')
ylabel('mean dW/dt, normalized to G_jensens')
...
title('growth, relative to Jensens expectations')
xlabel('fluctuating timescale')
ylabel('mean dW/dt / W, normalized to G_jensens')
