...
%  Goal: bin 4 of figure 23/24 sees extreme values. where do these come
%        from? plots to try to associate certain traits or timespoints with
%        dV/dt magnitudes greater than 50 cubic microns per hour.
...
%  commit: plot histograms to determine which bins extreme dV/dt magnitudes
%          appear in analysis. many appear at min trim if 3hr trimming
%          occurs BEFORE dV/dt calculation; they dissapear when trimming
%          occurs after
...
for i = 1:length(exptsToInclude)
    ...
    periodNum = floor(timeInPeriods);
    %timeInQuarters = ceil(timeInPeriodFraction * 4);
    
    
    % 11. find dV/dts with a magnitude higher than 50 cubic um/hr
    extreme_dVdts = dVdt_trim2(abs(dVdt) > 50);
    extreme_periods = periodNum(abs(dVdt) > 50);
    
    
    % 12. plot histograms of counts per bin
    shapes = {'o','*','square'};
    xmark = shapes{ec};
    if timescale == 300
        sp = 1;
    ...
    % plot dV/dt (mag > 50) vs period number
    figure(sp)
    histogram(extreme_periods,'FaceColor',color)
    hold on
    xlabel(strcat('Period number of (',num2str(36000/timescale),') total'))
    ylabel('number of |dV/dt| > 50 cubic um / hr')
    title('where do extreme dV/dt values appear in experiment?')
    %axis([0 36000/timescale 0 300])
