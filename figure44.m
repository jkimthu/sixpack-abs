...
%  Goal: checking for exponential growth via linear fits on semi-log plots
%        of individual volume
...
%%
    ...
    for condition = 1:4 % 1 = fluctuating; 3 = ave nutrient condition
        ...
        for lt = 1:length(tracks2Plot)
            ...
            plot(correctedTime/3600,log(volume),'Color',color)
            hold on
            if condition == 1
                ylim([0.8 2.8])
            elseif condition == 2
        ...
        elseif condition == 2
            ylabel('natural log volume (cubic um)')
        end
